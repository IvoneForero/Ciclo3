...
function seleccionarCloud(idCloud){
    //console.log(idCloud);
    $.ajax({
        url:"http://localhost:8080/api/Cloud/"+idCloud,
        type:"GET",
        datatype:"json",
        success: function (response) {
            $("#id").val(response.id);
...
function eliminarCloud(idCloud){
    let myData={
        id:idCloud
    };
...
function crearReservation(){
    ...
    var cloud = $("#cloud2").val();
    var client = $("#client2").val();

    if($("#startDate").val() == "" || $("#devolutionDate").val() == "" || $("#status").val() == 0 || $("#cloud2").val() == 0|| $("#client2").val() == 0) {
        alert("Faltan campos por llenar");
    }else{
        var data = {
            startDate:startDate, 
...
function crearMessage(){
    ...
    if($("#messagetext").val() == "" || $("#cloud").val() == 0 || $("#client").val() == 0){
        alert("Faltan campos por llenar");
    }else{
        var data = {
...
function cargarCloud(){
    $.ajax({
        ...
        success: function (response) {
            $("#cloud").empty();
            $("#cloud").append($("<option>").val(0).text("Seleccione una nube"));
            response.forEach(element => {
                var option = $("<option>");
                option.attr("value", element.id);
                option.text(element.name);
...
function cargarCloud2(){
    $.ajax({
        ...
        success: function (response) {
            $("#cloud2").empty();
            $("#cloud2").append("<option value=''>Seleccione una nube</option>");
            response.forEach(element => {
                var option = $("<option>");
                option.attr("value", element.id);
                option.text(element.name);
                $("#cloud2").append(option);
            });
        },
        error: function(xhr,status) {
            alert("Ocurrio un error en el consumo.");
